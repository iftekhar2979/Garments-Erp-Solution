...
import { useCreateUserMutation } from '../../../Redux/Features/api/Users/userApiSlice';
...
const SignUp = () => {
    ...
    const onsubmit = (obj) => {
        const {name,email,password,photoURL}=obj
     createNewUser(email,password)
     .then(result=>{
        const user=result.user
        createUser(obj)
        .then(res=>{
          const  notify=toast.success("created User Successfully")
          notify()
        }).catch(err=>{
