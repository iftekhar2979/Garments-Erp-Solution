...
const TBTable = ({ piNumbers, selectedValues, handleCheckboxChange, handlePi, matchedCompany, createdTB }) => {
    let { tbNumber, completed, companyName, piNumber = '', tbArray = [], } = piNumbers
    ...
    let content
    if (!createdTB.includes(tbNumber)) {
    ...
    return (
        <>
            ...
            <tr className={`border bg-gray-50 border text-cyan-950 hover:bg-green-400 dark:bg-gray-800 dark:border-gray-700 ${createdTB.includes(tbNumber) && 'bg-cyan-500 text-cyan-950 hover:bg-cyan-500'} ${selectedValues.includes(tbNumber) && 'bg-green-400'}`}>
                <th className='w-6'>
                ...
                <td className='text-md link flex tooltip'  onClick={handleModal}  data-tip={tbArray.join(' & ')} >{piNumber}
